...
export function getPatientLedger(patientId: number, from?: string, to?: string) {
  const db = getDb();
  ...
  const categories: Record<LedgerCategory, { count: number; billed: number; discount: number; paid: number; outstanding: number }> = {
  ...
  for (const e of timeline) {
    ...
    c.count++; c.billed += e.billed; c.discount += e.discount; c.paid += e.paid; c.outstanding += e.outstanding;
  }
