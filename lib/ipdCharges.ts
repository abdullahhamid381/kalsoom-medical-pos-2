...
export function syncAutoRoomCharge(
  db: any,
  admissionId: number,
  asOfDate: string,
  asOfTime: string,
  ...
  const adm = db.prepare(
    `SELECT a.admission_date, a.admission_time, r.price_per_day
     FROM admissions a JOIN rooms r ON r.id = a.room_id WHERE a.id = ?`
  ).get(admissionId) as { admission_date: string; admission_time: string; price_per_day: number } | undefined;
  if (!adm) return;
  ...
  db.prepare(`DELETE FROM admission_charges WHERE admission_id = ? AND charge_type = 'room' AND auto_generated = 1`).run(admissionId);
  db.prepare(`
...
export function recalcTotals(db: any, admissionId: number) {
  const agg = db.prepare(
    `SELECT charge_type, COALESCE(SUM(total),0) AS sub FROM admission_charges WHERE admission_id = ? GROUP BY charge_type`
  ).all(admissionId) as { charge_type: string; sub: number }[];
  const t: Record<string, number> = {};
  for (const r of agg) t[r.charge_type] = r.sub;
  ...
  db.prepare(`
    UPDATE admissions SET
      ...
      grand_total = ?, payment_status = ?, updated_at = datetime('now')
    WHERE id = ?
  `).run(
    t['room']||0, (t['medicine']||0)+(t['nursing']||0), t['lab']||0, t['procedure']||0,
    (t['doctor_fee']||0)+(t['other']||0), grand, pay_status, admissionId
  );
}
