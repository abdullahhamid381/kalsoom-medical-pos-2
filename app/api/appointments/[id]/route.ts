...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const appointment = db.prepare(`${appointmentSelect()} WHERE a.id = ?`).get(Number(params.id)) as any;
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (session.role === 'doctor') {
      if (existing.doctor_id !== session.doctorId) return fail('Not authorized to update this appointment.', 403);
      ...
      if (!STATUSES.includes(body.status)) return fail('Doctors can only update appointment status.');
      db.prepare(`UPDATE appointments SET status = ?, updated_at = datetime('now') WHERE id = ?`).run(
        body.status,
        id
      );
      const appointment = db.prepare(`${appointmentSelect()} WHERE a.id = ?`).get(id);
      return ok({ appointment });
    }
    ...
    if (typeof body.doctor_id !== 'undefined' && body.doctor_id !== null) {
      const doctor = db.prepare(`SELECT * FROM doctors WHERE id = ?`).get(Number(body.doctor_id)) as any;
    ...
    db.prepare(`UPDATE appointments SET ${updates.join(', ')} WHERE id = ?`).run(...values, id);
    const appointment = db.prepare(`${appointmentSelect()} WHERE a.id = ?`).get(id);
    return ok({ appointment });
  } catch (err) {
    return handleApiError(err);
