...
export async function GET(req: NextRequest) {
  try {
    ...
    if (session.role === 'doctor') {
      ...
      if (!session.doctorId) return ok({ appointments: [] });
    ...
    const appointments = db.prepare(sql).all(...values);
...
export async function POST(req: NextRequest) {
  try {
    const session = await requireSession();
    if (session.role === 'doctor') return fail('Doctors cannot create bookings - ask a receptionist.', 403);
    ...
    if (body.patient_id) {
      const existing = db.prepare(`SELECT id FROM patients WHERE id = ?`).get(Number(body.patient_id));
      if (!existing) return fail('Selected patient was not found.');
      ...
      if (!full_name || !phone) return fail('Patient name and phone number are required.');
      ...
      const result = db
        ...
        .run(
          full_name,
          phone,
          np.cnic ? String(np.cnic).trim() : null,
          np.age ? Number(np.age) : null,
          ['Male', 'Female', 'Other'].includes(np.gender) ? np.gender : 'Other',
          np.address ? String(np.address).trim() : null
        );
    ...
    const doctor = db.prepare(`SELECT * FROM doctors WHERE id = ? AND active = 1`).get(Number(body.doctor_id)) as
      | any
      | undefined;
    if (!doctor) return fail('Selected doctor was not found or is inactive.');

    const appointment_date = String(body.appointment_date || '').trim();
    const appointment_time = String(body.appointment_time || '').trim();
    if (!appointment_date || !appointment_time) return fail('Appointment date and time are required.');
    ...
    const payment_status =
      paid_amount >= amount - discount && amount - discount > 0
        ? 'paid'
        : paid_amount > 0
        ? 'partial'
        : 'unpaid';

    const appointment_no = nextAppointmentNo(db, appointment_date);
    const token_number = nextTokenNumber(db, doctor.id, appointment_date);
    ...
    const result = db
      ...
      .run(
        ...
        appointment_date,
        appointment_time,
        body.department ? String(body.department).trim() : doctor.department,
