...
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  ...
  const order = db.prepare(
    `SELECT lo.order_no, lo.patient_name, lo.reported_at, u.name AS reported_by_name
     FROM lab_orders lo LEFT JOIN users u ON u.id = lo.reported_by_user_id
     WHERE lo.verification_token = ? AND lo.report_status = 'reported'`
  ).get(params.token) as any;

  if (!order) return ok({ verified: false });
  return ok({
    verified: true,
