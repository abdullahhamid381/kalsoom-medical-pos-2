...
export async function GET(req: NextRequest) {
  try {
    ...
    if (q) {
      patients = db
        .prepare(
          `SELECT * FROM patients WHERE full_name LIKE ? OR phone LIKE ? OR cnic LIKE ? ORDER BY created_at DESC LIMIT 50`
        )
        .all(`%${q}%`, `%${q}%`, `%${q}%`);
    } else {
      patients = db.prepare(`SELECT * FROM patients ORDER BY created_at DESC LIMIT 50`).all();
    }
    return ok({ patients });
  } catch (err) {
