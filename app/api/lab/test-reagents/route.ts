...
export async function GET(req: NextRequest) {
    try {
        ...
        const vals: any[] = [];
        if (testId) {
            conds.push('tr.test_id = ?');
            vals.push(Number(testId));
        }
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!test_id || !reagent_id)
            return fail('test_id and reagent_id are required.');
        const db = await getDb();
        const result = await db.prepare(`INSERT INTO lab_test_reagents (test_id, reagent_id, qty_per_test) VALUES (?, ?, ?)
       ON CONFLICT(test_id, reagent_id) DO UPDATE SET qty_per_test = excluded.qty_per_test`).run(test_id, reagent_id, Number(b.qty_per_test || 1));
