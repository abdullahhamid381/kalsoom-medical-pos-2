...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (typeof b.is_culture === 'boolean') fields.is_culture = b.is_culture ? 1 : 0;
    ...
    db.prepare(`UPDATE lab_tests SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const id = Number(params.id);
    const used = db.prepare(`SELECT COUNT(*) AS c FROM lab_order_items WHERE test_id = ?`).get(id) as { c: number };
    if (used.c > 0) {
      db.prepare(`UPDATE lab_tests SET active = 0 WHERE id = ?`).run(id);
