...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const ranges = db.prepare(
      `SELECT * FROM lab_test_reference_ranges WHERE test_id = ? ORDER BY gender ASC, age_min ASC`
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const r = db.prepare(
      ...
      testId, b.gender || 'any', Number(b.age_min ?? 0), Number(b.age_max ?? 150), valueType,
