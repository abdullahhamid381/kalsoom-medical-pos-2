...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (b.status === 'collected' && !sample.collected_at) {
      updates.push('collected_at = datetime(\'now\')', 'collected_by_user_id = ?');
      vals.push(session.id);
    }
    if (b.status === 'received' && !sample.received_at) {
      updates.push('received_at = datetime(\'now\')', 'received_by_user_id = ?');
      vals.push(session.id);
    }
    ...
    db.prepare(`UPDATE lab_samples SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);

    // First sample collected bumps the parent order from pending -> processing.
    if (b.status === 'collected') {
      db.prepare(`UPDATE lab_orders SET status = 'processing' WHERE id = ? AND status = 'pending'`).run(sample.order_id);
    }

    const updated = db.prepare(`SELECT * FROM lab_samples WHERE id = ?`).get(id);
