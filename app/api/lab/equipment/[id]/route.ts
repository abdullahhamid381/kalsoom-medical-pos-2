...
export async function PUT(req: NextRequest, { params }: {
    ...
    try {
        ...
        await db.prepare(`UPDATE lab_equipment SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
...
export async function DELETE(_req: NextRequest, { params }: {
    ...
    try {
        await requireRole('super_admin');
        const db = await getDb();
        await db.prepare(`UPDATE lab_equipment SET active = 0 WHERE id = ?`).run(Number(params.id));
