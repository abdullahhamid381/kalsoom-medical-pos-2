...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const order = db.prepare(`SELECT lo.*, u.name AS booked_by_name FROM lab_orders lo JOIN users u ON u.id=lo.booked_by_user_id WHERE lo.id=?`).get(id) as any;
    ...
    db.prepare(`UPDATE lab_orders SET whatsapp_sent=1 WHERE id=?`).run(id);
