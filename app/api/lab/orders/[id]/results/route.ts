...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const items = db.prepare(
      `SELECT loi.*, lt.category AS test_category, lt.is_culture, lp.panel_name
       FROM lab_order_items loi
       JOIN lab_tests lt ON lt.id = loi.test_id
       LEFT JOIN lab_order_panels lp ON lp.id = loi.order_panel_id
       WHERE loi.order_id = ? ORDER BY loi.order_panel_id IS NULL DESC, loi.order_panel_id, loi.id`
    ).all(id) as any[];

    const rows = items.map((item) => {
      const existing = db.prepare(`SELECT * FROM lab_results WHERE order_item_id = ?`).get(item.id) as any;
      const range = resolveReferenceRange(db, item.test_id, order.patient_age, order.patient_gender);
      return {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (results.length === 0) return fail('No results provided.');
    ...
    if (order.reported_at) {
      for (const r of results) {
        const item = db.prepare(`SELECT * FROM lab_order_items WHERE id = ? AND order_id = ?`).get(Number(r.order_item_id), id) as any;
        if (!item) continue;
        const existing = db.prepare(`SELECT * FROM lab_results WHERE order_item_id = ?`).get(item.id) as any;
        if (!existing || !existing.entered_at) continue;
    ...
    const tx = db.transaction(() => {
      for (const r of results) {
        ...
        if (existing && existing.entered_at) {
          ...
          if (changed) {
            db.prepare(
              `INSERT INTO lab_result_amendments (result_id, old_value_numeric, old_value_text, new_value_numeric, new_value_text, reason, amended_by_user_id)
               VALUES (?, ?, ?, ?, ?, ?, ?)`
            ).run(existing.id, existing.value_numeric, existing.value_text, valueNumeric, valueText, r.reason || null, session.id);
          }
        }
        ...
        if (valueType === 'numeric' && valueNumeric != null && order.patient_id) {
          const prev = db.prepare(
            `SELECT lr.value_numeric FROM lab_results lr JOIN lab_orders lo ON lo.id = lr.order_id
             WHERE lo.patient_id = ? AND lr.test_id = ? AND lr.entered_at IS NOT NULL AND lo.id != ?
             ORDER BY lo.created_at DESC LIMIT 1`
          ).get(order.patient_id, item.test_id, id) as { value_numeric: number | null } | undefined;
        ...
        db.prepare(
          `INSERT INTO lab_results (order_id, order_item_id, test_id, value_type, value_numeric, value_text, unit,
           ...
           ON CONFLICT(order_item_id) DO UPDATE SET
             ...
             entered_at = datetime('now'), updated_at = datetime('now'), delta_flag = excluded.delta_flag`
        ).run(
          id, item.id, item.test_id, valueType, valueNumeric, valueText, r.unit || range?.unit || null,
          range?.id || null, formatRangeDisplay(range), flag, r.comment || null, session.id, deltaFlag
        );

        // Reagent auto-deduction: only once per order item, guarded by lab_order_items.reagent_deducted.
        if (!item.reagent_deducted) {
          const mappings = db.prepare(`SELECT * FROM lab_test_reagents WHERE test_id = ?`).all(item.test_id) as any[];
          for (const m of mappings) {
            const reagent = db.prepare(`SELECT * FROM lab_reagents WHERE id = ?`).get(m.reagent_id) as any;
            if (!reagent) continue;
            const qtyBefore = reagent.stock_qty;
            const qtyAfter = qtyBefore - m.qty_per_test;
            db.prepare(`UPDATE lab_reagents SET stock_qty = ? WHERE id = ?`).run(qtyAfter, reagent.id);
            db.prepare(
              `INSERT INTO lab_reagent_movements (reagent_id, movement_type, qty_change, qty_before, qty_after, reference_no, created_by_user_id)
               VALUES (?, 'consumption', ?, ?, ?, ?, ?)`
            ).run(reagent.id, -m.qty_per_test, qtyBefore, qtyAfter, String(item.id), session.id);
          }
          if (mappings.length > 0) {
            db.prepare(`UPDATE lab_order_items SET reagent_deducted = 1 WHERE id = ?`).run(item.id);
          }
        }
      }
