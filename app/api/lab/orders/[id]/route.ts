...
async function getOrder(db: any, id: number) {
    const order = await db.prepare(`SELECT lo.*, u.name AS booked_by_name,
     ...
     WHERE lo.id = ?`).get(id) as any;
    if (!order)
        return null;
    const items = await db.prepare(`SELECT * FROM lab_order_items WHERE order_id = ? ORDER BY id`).all(id);
    const panels = await db.prepare(`SELECT * FROM lab_order_panels WHERE order_id = ? ORDER BY id`).all(id);
    const samples = await db.prepare(`SELECT * FROM lab_samples WHERE order_id = ? ORDER BY id`).all(id) as any[];
    for (const sample of samples) {
        sample.items = items.filter((i: any) => i.sample_id === sample.id);
    }
    const homeCollection = order.collection_mode === 'home'
        ? await db.prepare(`SELECT * FROM lab_home_collections WHERE order_id = ?`).get(id) : null;
...
export async function GET(_req: NextRequest, { params }: {
    ...
    try {
        ...
        const order = await getOrder(db, Number(params.id));
        if (!order)
...
export async function PUT(req: NextRequest, { params }: {
    ...
    try {
        ...
        const existing = await db.prepare(`SELECT * FROM lab_orders WHERE id = ?`).get(id) as any;
        if (!existing)
            return fail('Order not found.', 404);
        ...
        if (PAY_METHODS.includes(b.payment_method)) {
            updates.push('payment_method = ?');
            vals.push(b.payment_method);
        }
        if (b.referring_doctor !== undefined) {
            updates.push('referring_doctor = ?');
            vals.push(b.referring_doctor);
        }
        ...
        if (b.insurance_provider !== undefined) {
            updates.push('insurance_provider = ?');
            vals.push(b.insurance_provider || null);
        }
        if (b.tpa_reference !== undefined) {
            updates.push('tpa_reference = ?');
            vals.push(b.tpa_reference || null);
        }
        if (CLAIM_STATUSES.includes(b.claim_status)) {
            updates.push('claim_status = ?');
            vals.push(b.claim_status);
        }
        ...
        await db.prepare(`UPDATE lab_orders SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
        return ok({ order: await getOrder(db, id) });
    }
    catch (err) {
        return handleApiError(err);
    }
...
export async function DELETE(_req: NextRequest, { params }: {
    ...
    try {
        ...
        const db = await getDb();
        await db.prepare(`DELETE FROM lab_orders WHERE id = ?`).run(Number(params.id));
