...
export async function POST(req: NextRequest, { params }: {
    ...
    try {
        ...
        if (order.report_status !== 'reported')
            return fail('Finalize the report before sending it.');
        if (!order.patient_phone)
            return fail('This patient has no phone number on file.');
        const clinic = getClinicInfo();
        const data = await buildLabReportData(db, id, req.nextUrl.origin, clinic);
        if (!data)
            return fail('Order not found.', 404);
        ...
        if (sendResult.ok) {
            await db.prepare(`UPDATE lab_orders SET report_whatsapp_sent = 1, report_whatsapp_sent_at = now_iso() WHERE id = ?`).run(id);
            return ok({ sent: true });
        }
