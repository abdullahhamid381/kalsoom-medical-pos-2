...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    for (const item of items) {
      ...
      ).all(item.order_item_id) as any[];
      item.culture = organisms.map((org) => ({
        organism: org.organism_name,
        growth_count: org.growth_count,
        sensitivities: db.prepare(
          `SELECT a.name AS antibiotic, cs.result FROM lab_culture_sensitivities cs
