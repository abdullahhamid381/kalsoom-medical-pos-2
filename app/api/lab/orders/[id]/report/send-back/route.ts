...
export async function POST(_req: NextRequest, { params }: {
    ...
    try {
        ...
        if (order.report_status !== 'reviewed')
            return fail('Only a reviewed report can be sent back to the technician.');
        await db.prepare(`UPDATE lab_orders SET report_status = 'entered', reviewed_by_user_id = NULL, reviewed_at = NULL WHERE id = ?`).run(id);
