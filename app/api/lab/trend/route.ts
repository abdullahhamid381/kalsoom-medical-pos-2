  ...
  try {
    ...
    if (testId) {
      const rows = db.prepare(
        `SELECT lr.value_type, lr.value_numeric, lr.value_text, lr.unit, lr.flag, lr.entered_at, lo.order_no, lo.created_at
         FROM lab_results lr
      ...
      ).all(patientId, testId);
      return ok({ history: rows });
    }

    // No test_id: return every test the patient has ever had results for, grouped.
    const rows = db.prepare(
      `SELECT lr.test_id, loi.test_name, lr.value_type, lr.value_numeric, lr.value_text, lr.unit, lr.flag,
    ...
    const byTest = new Map<number, { test_id: number; test_name: string; unit: string | null; history: any[] }>();
    for (const row of rows) {
      if (!byTest.has(row.test_id)) byTest.set(row.test_id, { test_id: row.test_id, test_name: row.test_name, unit: row.unit, history: [] });
