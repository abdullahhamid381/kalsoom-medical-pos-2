...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    db.prepare(`UPDATE suppliers SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);

    return ok({ updated: true, supplier: db.prepare(`SELECT * FROM suppliers WHERE id = ?`).get(id) });
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const id = Number(params.id);
    const used = db.prepare(`SELECT COUNT(*) AS c FROM purchase_orders WHERE supplier_id = ?`).get(id) as { c: number };
    if (used.c > 0) {
