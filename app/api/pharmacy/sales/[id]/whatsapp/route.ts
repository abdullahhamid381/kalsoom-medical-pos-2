...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const sale = db.prepare(`SELECT ps.*, u.name AS sold_by_name FROM pharmacy_sales ps JOIN users u ON u.id=ps.sold_by_user_id WHERE ps.id=?`).get(id) as any;
    if (!sale) return fail('Sale not found.', 404);
    const items = db.prepare(`SELECT * FROM pharmacy_sale_items WHERE sale_id=? ORDER BY id`).all(id) as any[];
    ...
    db.prepare(`UPDATE pharmacy_sales SET whatsapp_sent=1 WHERE id=?`).run(id);
