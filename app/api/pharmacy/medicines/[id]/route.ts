...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const med = db.prepare(`SELECT * FROM medicines WHERE id = ?`).get(id) as any;
    if (!med) return fail('Medicine not found.', 404);
    ...
    if (typeof b.prescription_required === 'boolean') fields.prescription_required = b.prescription_required ? 1 : 0;
      ...
      if (diff !== 0) {
        ...
        db.prepare(
          `INSERT INTO stock_movements (medicine_id, medicine_name, movement_type, location, qty_change, qty_before, qty_after, reference_no, notes, created_by_user_id)
           VALUES (?, ?, ?, 'pharmacy', ?, ?, ?, ?, ?, ?)`
        ).run(id, med.name, mtype, diff, med.stock_qty, newQty, b.reference_no||null, b.notes||null, session.id);
      }
    ...
    if (b.stock_qty_store !== undefined) {
      ...
      if (diff !== 0) {
        ...
        db.prepare(
          `INSERT INTO stock_movements (medicine_id, medicine_name, movement_type, location, qty_change, qty_before, qty_after, reference_no, notes, created_by_user_id)
           VALUES (?, ?, ?, 'store', ?, ?, ?, ?, ?, ?)`
        ).run(id, med.name, mtype, diff, med.stock_qty_store, newQty, b.reference_no||null, b.notes||null, session.id);
      }
    ...
    db.prepare(`UPDATE medicines SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);

    return ok({ updated: true, medicine: db.prepare(`SELECT * FROM medicines WHERE id = ?`).get(id) });
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const id = Number(params.id);
    const used = db.prepare(`SELECT COUNT(*) AS c FROM pharmacy_sale_items WHERE medicine_id = ?`).get(id) as { c: number };
    if (used.c > 0) {
      db.prepare(`UPDATE medicines SET active = 0 WHERE id = ?`).run(id);
      return ok({ deactivated: true });
    }
    db.prepare(`DELETE FROM stock_movements WHERE medicine_id = ?`).run(id);
