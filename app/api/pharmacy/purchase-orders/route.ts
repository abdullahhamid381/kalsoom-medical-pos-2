...
export async function GET(req: NextRequest) {
    try {
        ...
        if (from) {
            conds.push('LEFT(po.created_at, 10) >= ?');
            vals.push(from);
        }
        if (to) {
            conds.push('LEFT(po.created_at, 10) <= ?');
        ...
        const orders = await db.prepare(`${PO_SELECT} ${where} ORDER BY po.created_at DESC LIMIT 500`).all(...vals);
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!Array.isArray(b.items) || b.items.length === 0)
            return fail('Add at least one medicine to the order.');
        const db = await getDb();
        const supplier = await db.prepare(`SELECT * FROM suppliers WHERE id = ? AND active = 1`).get(Number(b.supplier_id)) as any;
        if (!supplier)
            return fail('Supplier not found or inactive.');
        ...
        const po_no = await nextPoNo(db, dateStr);
        ...
        const tx = db.transaction(async () => {
            const result = await db.prepare(`INSERT INTO purchase_orders (po_no, supplier_id, status, expected_date, subtotal, discount, total, notes, created_by_user_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(po_no, supplier.id, status, b.expected_date || null, subtotal, discount, total, b.notes || null, session.id);
            ...
            for (const item of b.items) {
                ...
                await db.prepare(`INSERT INTO purchase_order_items (po_id, medicine_id, medicine_name, ordered_qty, unit_price, total)
           VALUES (?, ?, ?, ?, ?, ?)`).run(poId, med.id, med.name, Number(item.ordered_qty), Number(item.unit_price), lineTotal);
            }
            return poId;
        });
        const poId = await tx();
