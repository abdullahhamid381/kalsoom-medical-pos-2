...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (po.status === 'received' || po.status === 'cancelled') return fail(`Purchase order is already ${po.status}.`);

    const b = await req.json();
    if (!Array.isArray(b.items) || b.items.length === 0) return fail('Specify at least one line to receive.');

    // Validate every line against remaining ordered qty
    for (const line of b.items) {
      const qty = Number(line.qty || 0);
      if (qty <= 0) continue;
      const poItem = db.prepare(`SELECT * FROM purchase_order_items WHERE id = ? AND po_id = ?`).get(Number(line.po_item_id), poId) as any;
      if (!poItem) return fail(`Purchase order line ${line.po_item_id} not found.`);
      const remaining = poItem.ordered_qty - poItem.received_qty;
      if (qty > remaining) return fail(`Cannot receive ${qty} of ${poItem.medicine_name} — only ${remaining} remaining on this order.`);
      if (!String(line.batch_no || '').trim()) return fail(`Batch number is required for ${poItem.medicine_name}.`);
      if (!String(line.expiry_date || '').trim()) return fail(`Expiry date is required for ${poItem.medicine_name}.`);
    }
    ...
    const tx = db.transaction(() => {
      for (const line of b.items) {
        const qty = Number(line.qty || 0);
        if (qty <= 0) continue;
        const poItem = db.prepare(`SELECT * FROM purchase_order_items WHERE id = ?`).get(Number(line.po_item_id)) as any;
        ...
        const batchResult = db.prepare(
          ...
          med.id, String(line.batch_no).trim(), line.expiry_date, line.mfg_date || null, qty,
          poItem.unit_price, Number(line.sale_price || med.sale_price || 0), location,
          po.supplier_id, poId, po.po_no, session.id
        );
        ...
        db.prepare(
          `INSERT INTO stock_movements (medicine_id, medicine_name, movement_type, location, qty_change, qty_before, qty_after, reference_no, notes, batch_id, created_by_user_id)
           VALUES (?, ?, 'purchase', ?, ?, ?, ?, ?, ?, ?, ?)`
        ).run(med.id, med.name, location, qty, qty_before, qty_after, po.po_no, `Received against ${po.po_no}`, batchId, session.id);
      }
