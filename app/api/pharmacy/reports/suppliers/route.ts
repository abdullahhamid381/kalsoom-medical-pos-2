...
export async function GET(req: NextRequest) {
  try {
    ...
    const suppliers = db.prepare(`SELECT * FROM suppliers WHERE active = 1 ORDER BY name ASC`).all() as any[];

    // Range-scoped activity, grouped per supplier
    const receivedInRange = db.prepare(`
    ...
    `).all(from, to) as { supplier_id: number; total: number }[];
    const paidInRange = db.prepare(`
      SELECT supplier_id, COALESCE(SUM(amount), 0) AS total FROM supplier_payments
      WHERE date(created_at) BETWEEN ? AND ? GROUP BY supplier_id
    `).all(from, to) as { supplier_id: number; total: number }[];
    const returnedInRange = db.prepare(`
    ...
    const receivedAllTime = db.prepare(`
      SELECT po.supplier_id, COALESCE(SUM(poi.received_qty * poi.unit_price), 0) AS total
      FROM purchase_order_items poi JOIN purchase_orders po ON po.id = poi.po_id
      GROUP BY po.supplier_id
    `).all() as { supplier_id: number; total: number }[];
    const paidAllTime = db.prepare(`SELECT supplier_id, COALESCE(SUM(amount), 0) AS total FROM supplier_payments GROUP BY supplier_id`).all() as { supplier_id: number; total: number }[];
    const returnedAllTime = db.prepare(`SELECT supplier_id, COALESCE(SUM(total), 0) AS total FROM purchase_returns GROUP BY supplier_id`).all() as { supplier_id: number; total: number }[];

    const toMap = (rows: { supplier_id: number; total: number }[]) => new Map(rows.map(r => [r.supplier_id, r.total]));
