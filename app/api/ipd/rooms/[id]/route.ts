...
export async function PUT(req: NextRequest, { params }: {
    ...
    try {
        ...
        await db.prepare(`UPDATE rooms SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
...
export async function DELETE(_req: NextRequest, { params }: {
    ...
    try {
        ...
        const db = await getDb();
        const id = Number(params.id);
        const used = await db.prepare(`SELECT COUNT(*) AS c FROM admissions WHERE room_id = ?`).get(id) as {
            c: number;
        };
        if (used.c > 0) {
            await db.prepare(`UPDATE rooms SET active = 0 WHERE id = ?`).run(id);
