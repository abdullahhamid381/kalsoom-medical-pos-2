...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const adm = db.prepare(`SELECT a.*, p.full_name AS patient_name, p.phone AS patient_phone, r.room_no, r.room_type, d.name AS doctor_name, u.name AS admitted_by_name FROM admissions a JOIN patients p ON p.id=a.patient_id JOIN rooms r ON r.id=a.room_id LEFT JOIN doctors d ON d.id=a.doctor_id JOIN users u ON u.id=a.admitted_by_user_id WHERE a.id=?`).get(id) as any;
    ...
    db.prepare(`UPDATE admissions SET whatsapp_sent=1 WHERE id=?`).run(id);
