...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const charges = db.prepare(
      `SELECT ac.*, u2.name AS added_by_name FROM admission_charges ac
       JOIN users u2 ON u2.id = ac.added_by_user_id
       WHERE ac.admission_id = ? ORDER BY ac.charge_date, ac.created_at`
    ).all(id) as any[];
    ...
    await new Promise<void>(resolve => {
      ...
      y += 14;
      doc.moveTo(40, y).lineTo(555, y).strokeColor('#e5e7eb').lineWidth(1).stroke(); y += 6;
      ...
      doc.moveTo(310, ty).lineTo(545, ty).strokeColor('#d1d5db').stroke(); ty += 4;
      ...
      ty += 14;
      if (adm.discount > 0) {
        doc.fillColor('#d62828').font('Helvetica').fontSize(9).text('Discount:', 310, ty, { width:130 }).text(`-Rs. ${adm.discount}`, 445, ty, { width:100, align:'right' }); ty += 13;
      }
