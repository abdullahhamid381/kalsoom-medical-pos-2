...
function getCharges(db: any, id: number) {
  return db.prepare(
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    let admission = db.prepare(`${SEL} WHERE a.id = ?`).get(id) as any;
    if (!admission) return fail('Admission not found.', 404);
    ...
    if (admission.status === 'admitted') {
      ...
      syncAutoRoomCharge(db, id, date, time, session.id);
      admission = db.prepare(`${SEL} WHERE a.id = ?`).get(id);
    }

    const charges = getCharges(db, id);
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = db.prepare(`SELECT * FROM admissions WHERE id = ?`).get(id) as any;
    if (!existing) return fail('Admission not found.', 404);

    const updates: string[] = [];
    const vals: any[] = [];
    if (b.diagnosis !== undefined) { updates.push('diagnosis = ?'); vals.push(b.diagnosis); }
    if (b.notes !== undefined)     { updates.push('notes = ?'); vals.push(b.notes); }
    if (b.doctor_id !== undefined) { updates.push('doctor_id = ?'); vals.push(b.doctor_id); }
    if (b.payment_method) { updates.push('payment_method = ?'); vals.push(b.payment_method); }
    if (b.discount !== undefined) {
      updates.push('discount = ?'); vals.push(Number(b.discount));
    }
    ...
    db.prepare(`UPDATE admissions SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
    const admission = db.prepare(`${SEL} WHERE a.id = ?`).get(id);
    const charges = getCharges(db, id);
    return ok({ admission, charges });
  } catch (err) { return handleApiError(err); }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const id = Number(params.id);
    const adm = db.prepare(`SELECT * FROM admissions WHERE id = ?`).get(id) as any;
    if (!adm) return fail('Admission not found.', 404);
    if (adm.status === 'admitted') db.prepare(`UPDATE rooms SET status = 'available' WHERE id = ?`).run(adm.room_id);
