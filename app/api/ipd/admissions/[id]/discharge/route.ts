...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (adm.status !== 'admitted') return fail('Patient is not currently admitted.');
    ...
    syncAutoRoomCharge(db, id, dischargeDate, dischargeTime, session.id);

    // Aggregate charges by type
    const chargeAgg = db.prepare(`
      SELECT charge_type, COALESCE(SUM(total), 0) AS subtotal
      FROM admission_charges WHERE admission_id = ? GROUP BY charge_type
    `).all(id) as { charge_type: string; subtotal: number }[];

    const totals: Record<string, number> = {};
    for (const row of chargeAgg) totals[row.charge_type] = row.subtotal;
    ...
    const tx = db.transaction(() => {
      db.prepare(`
        ...
        WHERE id = ?
      `).run(
        dischargeDate, dischargeTime, daysStayed,
        totals['room'] || 0, (totals['medicine'] || 0) + (totals['nursing'] || 0),
        totals['lab'] || 0, totals['procedure'] || 0,
        (totals['doctor_fee'] || 0) + (totals['other'] || 0),
        grand_total, discount, paid_amount, payment_status,
        b.payment_method || 'cash', id
      );
      // Free the room
      db.prepare(`UPDATE rooms SET status = 'available' WHERE id = ?`).run(adm.room_id);
    });
    tx();
