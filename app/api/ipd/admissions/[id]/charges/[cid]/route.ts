...
export async function PUT(req: NextRequest, { params }: { params: { id: string; cid: string } }) {
  try {
    ...
    const charge = db.prepare(`SELECT * FROM admission_charges WHERE id = ?`).get(cid) as any;
    if (!charge) return fail('Charge not found.', 404);
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(
      b.description || charge.description, qty, unit_price, total,
      b.charge_date || charge.charge_date, cid
    );
    recalcTotals(db, Number(params.id));
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string; cid: string } }) {
  try {
    ...
    db.prepare(`DELETE FROM admission_charges WHERE id = ?`).run(cid);
    recalcTotals(db, Number(params.id));
