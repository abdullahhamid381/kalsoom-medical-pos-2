...
export async function GET(_req: NextRequest, { params }: {
    ...
    try {
        ...
        const charges = await db.prepare(`SELECT ac.*, u.name AS added_by_name FROM admission_charges ac
       JOIN users u ON u.id = ac.added_by_user_id
       WHERE ac.admission_id = ? ORDER BY ac.charge_date ASC, ac.created_at ASC`).all(Number(params.id));
        return ok({ charges });
    }
    catch (err) {
...
export async function POST(req: NextRequest, { params }: {
    ...
    try {
        ...
        if (!TYPES.includes(b.charge_type))
            return fail('Invalid charge type.');
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(admissionId, b.charge_type, b.description.trim(), qty, unit_price, total, b.charge_date || new Date().toISOString().slice(0, 10), b.ref_sale_id || null, b.ref_order_id || null, session.id);
