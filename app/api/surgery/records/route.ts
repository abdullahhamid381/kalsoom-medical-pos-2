...
export async function GET(req: NextRequest) {
  try {
    ...
    const conds: string[] = [];
    const vals: any[] = [];
    if (sp.get('patient_id')) { conds.push('sr.patient_id = ?'); vals.push(Number(sp.get('patient_id'))); }
    if (sp.get('status')) { conds.push('sr.status = ?'); vals.push(sp.get('status')); }
    if (sp.get('from')) { conds.push('sr.surgery_date >= ?'); vals.push(sp.get('from')); }
    if (sp.get('to'))   { conds.push('sr.surgery_date <= ?'); vals.push(sp.get('to')); }
    if (sp.get('q')) { conds.push('(p.full_name LIKE ? OR p.phone LIKE ? OR sr.surgery_no LIKE ?)'); const q = `%${sp.get('q')}%`; vals.push(q,q,q); }
    const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
    const records = db.prepare(`${SEL} ${where} ORDER BY sr.surgery_date DESC LIMIT 500`).all(...vals);
    return ok({ records });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!b.surgery_type_id) return fail('Select a surgery type.');
    const db = getDb();
    const stype = db.prepare(`SELECT * FROM surgery_types WHERE id = ?`).get(Number(b.surgery_type_id)) as any;
    if (!stype) return fail('Surgery type not found.');
    const dateStr = b.surgery_date || new Date().toISOString().slice(0, 10);
    const surgery_no = nextSurgeryNo(db, dateStr);
    ...
    const r = db.prepare(`
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).run(surgery_no,b.patient_id,b.admission_id||null,b.surgery_type_id,b.surgeon_id||null,
      b.anesthetist||null,dateStr,b.surgery_time||null,Number(b.duration_hrs||stype.duration_hrs||1),
      b.theatre_no||null,b.diagnosis||null,b.procedure_notes||null,
      sf,af,tf,mc,oc,total_cost,discount,paid,ps,b.payment_method||'cash',
      b.status||'scheduled',b.outcome||null,session.id);
    const record = db.prepare(`${SEL} WHERE sr.id = ?`).get(r.lastInsertRowid);
