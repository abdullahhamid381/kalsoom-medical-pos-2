...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const rec = db.prepare(`${SEL} WHERE sr.id = ?`).get(Number(params.id));
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = db.prepare(`SELECT * FROM surgery_records WHERE id = ?`).get(id) as any;
    ...
    updates.push('total_cost = ?', 'payment_status = ?', "updated_at = datetime('now')");
    vals.push(total_cost, payment_status);
    db.prepare(`UPDATE surgery_records SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
    return ok({ record: db.prepare(`${SEL} WHERE sr.id = ?`).get(id) });
  } catch (err) { return handleApiError(err); }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    db.prepare(`DELETE FROM surgery_records WHERE id = ?`).run(Number(params.id));
