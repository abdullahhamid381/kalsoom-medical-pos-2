...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    db.prepare(`UPDATE surgery_types SET ${updates.join(', ')} WHERE id = ?`).run(...vals, id);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const id = Number(params.id);
    const used = db.prepare(`SELECT COUNT(*) AS c FROM surgery_records WHERE surgery_type_id = ?`).get(id) as { c: number };
    if (used.c > 0) { db.prepare(`UPDATE surgery_types SET active = 0 WHERE id = ?`).run(id); return ok({ deactivated: true }); }
    db.prepare(`DELETE FROM surgery_types WHERE id = ?`).run(id);
